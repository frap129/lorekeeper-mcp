...
## Overview
Defines the five core MCP (Model Context Protocol) tools that enable AI assistants to query D&D 5e game data through the LoreKeeper server.
...
## Notes
- Tools use existing API clients; no direct HTTP calls in tool code
- FastMCP handles parameter validation automatically
- Cache layer is transparent to tool implementations
- Tool specifications match `docs/tools.md` documentation
